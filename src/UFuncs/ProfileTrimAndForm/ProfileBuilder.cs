...
namespace TSG_Library.Utilities
{
    public static class ProfileBuilder
    {
        ...
        private static IEnumerable<Pad> CreateLower(UniteTarget[] uniteTargets, int color, int layer)
        {
            ...
            {
                ExtractFace linkedBody = WaveLinkObject(unite.Unite.GetBodies()[0], ExtractFaceBuilder.ParentPartType.WorkPart);
        ...
        private static void CreatePunches(IList<UniteTarget> uniteTargets, int color, int layer)
        {
            ...
            foreach (SlugObj slug in unite.Slugs)
                try
                {
                    ...
                    EdgeBoundaryRule edgeBoundaryRule = __work_part_.ScRuleFactory.CreateRuleEdgeBoundary(new[] { slug.TopFace });
            ...
            string leftHandSide)
        {
            ...
            using (session_.__UsingBuilderDestroyer(extrudeBuilder))
            {
                ...
                {
                    CurveDumbRule curveDumbRule = __work_part_.ScRuleFactory.CreateRuleBaseCurveDumb(new IBaseCurve[] { curve });
